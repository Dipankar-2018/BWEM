...
//file processing needed
    //'passbook_file',
        $passbook_filename=$_FILES['passbook_file']['name'];
            ...
            if(in_array($fileext1, $fileextStor1))
				{
					$passbook=$registration_no."_ng_bank_".$group_name.".".$fileext1;
    ...
    //'registration_certificate_file'
        $registration_certificate_filename=$_FILES['registration_file']['name'];
            ...
            if(in_array($fileext2, $fileextStor2))
				{
					$registrationCertificate=$registration_no."_ng_reg_".$group_name.".".$fileext2;
  ...
  $result=$obj->insertData('ngo',$condition_arr);
  //for groupmember table
  $member_name =$_POST['member_name'];
  $member_gender =$_POST['member_gender'];
  $member_age =$_POST['member_age'];
  $member_qualification =$_POST['member_qualification'];
  //$id=$obj->getData('entrepreneur','id',array('group_name'=>$group_name),'id','desc','1')[0]['id'];
    for($i=0;$i<count($member_name);$i++){
        ...
        $result=$obj->insertData('ngo_members',$condition_arr);
    }
    if($obj->getData('ngo','count(id)',array('group_name'=>$group_name))[0]["count(id)"]=="1"){
...
if(isset($_SESSION['login'])&&$_SESSION['login']==true){
    header("location:../data.php?cat=ng&dist=".strtolower($dist));
}else{
    header("location:../../forms/ngo.php");
}
