...
if(isset($_POST['submit'])){
  ...
  $formID= $obj->getFormId("ENTP","entrepreneur");
...
//file processing needed
    //'passbook_file',
        $passbook_filename=$_FILES['passbook_file']['name'];
            ...
            if(in_array($fileext1, $fileextStor1))
				{
					$passbook=$formID."_ep_bank.".$fileext1;
    ...
    //'registration_certificate_file'
        $registration_certificate_filename=$_FILES['registration_file']['name'];
            ...
            if(in_array($fileext2, $fileextStor2))
				{
					$registrationCertificate=$formID."_ep_reg.".$fileext2;
  ...
  $result=$obj->insertData('entrepreneur',$condition_arr);
  ...
  //$id=$obj->getData('entrepreneur','id',array('group_name'=>$group_name),'id','desc','1')[0]['id'];
    for($i=0;$i<count($member_name);$i++){
        ...
        $result=$obj->insertData('entrepreneur_members',$condition_arr);
    }
...
if(isset($_SESSION['login'])&&$_SESSION['login']==true){
    header("location:../data.php?cat=ep&dist=".strtolower($dist));
}else{
    header("location:../../forms/entrepreneur.php");
}
